...
(cd "$ROOT_DIR/backend"&&npm start)& backend_pid=$!;(cd "$ROOT_DIR/frontend"&&BROWSER=none PORT="$FRONTEND_PORT" npm start)& frontend_pid=$!;cleanup(){ kill "$backend_pid" "$frontend_pid" 2>/dev/null||true;wait "$backend_pid" "$frontend_pid" 2>/dev/null||true;};trap cleanup EXIT INT TERM;wait "$backend_pid" "$frontend_pid"
